...
class OddsAPIClient:
    ...
    def get_player_props(self, days=2):
        ...
        print(f"Total props: {len(all_props)} (over {days} day(s))")
        return all_props
    ...
    def _fetch_props_for_date(self, date):
        ...
        try:
            ...
            print(f"Games for {date.strftime('%Y-%m-%d')}: {len(games)}")
